...
import com.ourgiant.docscrubber.util.NetworkFetchException;
...
/** App name, version, and an update check against GitHub releases — the only place in the app that makes a network call. */
final class AboutDialog extends JDialog {
    ...
    /**
     * MainWindow's silent startup check already knows {@code knownNewerRelease} — skips a
     * second, redundant network call and shows it immediately instead of flashing "Checking
     * for updates..." first. Pass {@code null} to check live instead, same as the single-arg
     * constructor.
     * <p>
    ...
    AboutDialog(Frame parent, UpdateChecker.ReleaseInfo knownNewerRelease) {
        super(parent, "About DocScrubber", knownNewerRelease == null);
    ...
    private static String buildHtml(String currentVersion) {
        return """
            ...
            <h2 style="margin-top: 0;">DocScrubber</h2>
            <p>Version %s</p>
            <p>Scans PDF, Word, and plain-text documents for prompt-injection
            &quot;poison pills&quot; &mdash; instructions hidden from human
            readers but meant for whatever AI tool opens the document next.</p>
            <p>&copy; OurGiant</p>
            </body></html>
